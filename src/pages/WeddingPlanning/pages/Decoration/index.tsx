...
const DecorationPage = () => {
    ...
    const { decoration } = useWedding();
    const utilityType: Utility = Utility.DECORATION;
    ...
    return (
        <>
            ...
            <Stack w={'6xl'} mx={'auto'} my={10} gap={5} pos={'relative'}>
                ...
                {decoration.length === 0 && (
                    <Button pos={'absolute'} top={2} right={0} variant={'outline'} onClick={() => navigate(`/wedding-planning/${param.budget}/restaurants`)}>
                        Bỏ qua
                    </Button>
                )}
                {!isLoading ? (
                        ...
                        {products.length > 0 ? (
                            ...
                            >
                                {products.map(product => (
                                    <ProductItem type="decoration" product={product} />
                                ))}
                            </SimpleGrid>
                        ) : (
                ...
                <HStack justify={'space-between'}>
                    ...
                    <Button variant={'ghost'} rightIcon={<ArrowForward />} onClick={() => navigate(`/wedding-planning/${param.budget}/restaurants`)}>
                        {decoration.length > 0 ? 'Bước tiếp theo' : 'Bỏ qua'}
                    </Button>
                </HStack>
            </Stack>
        </>

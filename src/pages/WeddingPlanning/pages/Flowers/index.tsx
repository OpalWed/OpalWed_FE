...
const FlowersPage = () => {
    ...
    const { flowers } = useWedding();
    const utilityType: Utility = Utility.FLOWERS;
    ...
    return (
        <>
            ...
            <Stack w={'6xl'} mx={'auto'} my={10} gap={5} pos={'relative'}>
                ...
                {flowers.length === 0 && (
                    <Button pos={'absolute'} top={2} right={0} variant={'outline'} onClick={() => navigate(`/wedding-planning/${param.budget}/photography`)}>
                        Bỏ qua
                    </Button>
                )}
                {!isLoading ? (
                        ...
                        {products.length > 0 ? (
                            ...
                            >
                                {products.map(product => (
                                    <ProductItem type="flowers" product={product} />
                                ))}
                            </SimpleGrid>
                        ) : (
                ...
                <HStack justify={'space-between'}>
                    ...
                    <Button variant={'ghost'} rightIcon={<ArrowForward />} onClick={() => navigate(`/wedding-planning/${param.budget}/photography`)}>
                        {flowers.length > 0 ? 'Bước tiếp theo' : 'Bỏ qua'}
                    </Button>
                </HStack>
            </Stack>
        </>
